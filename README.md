...
`framepick.exe`를 실행하면 됩니다. 별도 설치가 필요 없습니다.
첫 실행 시 필요한 도구(ffmpeg, yt-dlp)가 자동으로 다운로드됩니다.

## 개발 환경 빌드

> 아래는 소스 코드에서 직접 빌드하려는 개발자를 위한 안내입니다.

### 사전 요구사항

- **Rust 1.70+**: [https://rustup.rs](https://rustup.rs) 에서 설치
- **Tauri CLI**: `cargo install tauri-cli` 로 설치
