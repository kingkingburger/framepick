//! Generates slides.html with a dark-themed viewer layout.
//!
//! The output is a fully standalone HTML file with:
//! - Sidebar TOC (table of contents) with timestamp navigation
//! - Main content area displaying captured frames with subtitle text
//! - Keyboard navigation (←/→, j/k) between slides
//! - Responsive layout that collapses sidebar on narrow screens
//! - Dark theme consistent with the app UI
...
/// A single captured frame segment.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct Segment {
    /// Zero-based index of the segment.
    pub index: usize,
    /// Display timestamp, e.g. "00:01:23".
    pub timestamp: String,
    /// Subtitle / caption text for this frame.
    pub text: String,
    /// Image filename (relative to `images/` directory), e.g. "frame_0001_00-01-23.jpg".
    pub image: String,
}
...
impl Segment {
    /// Create a segment from a captured frame with subtitle text.
    ///
    /// Used when subtitles are available (subtitle capture mode).
    ...
    /// Create a segment from a captured frame without subtitle text.
    ///
    /// The text field is set to the timestamp in bracket format,
    /// e.g. `[00:01:30]`, which serves as a visual marker in the
    /// slides viewer for frames captured via scene-change or
    /// fixed-interval modes.
...
/// Convert a list of captured frames into segments without subtitle text.
///
/// Each segment's text is set to `[HH:MM:SS]` timestamp format.
/// Used for scene-change and fixed-interval capture modes where
/// no subtitle cues are available.
...
/// Convert captured frames into segments using subtitle cue text.
///
/// Matches each frame to the closest subtitle cue by start time
/// (within a 2-second tolerance window). Frames without a matching
/// cue fall back to the `[HH:MM:SS]` timestamp format.
...
/// Check if text is a timestamp-only marker (e.g. "[00:01:30]").
///
/// Returns true for text matching the pattern `[HH:MM:SS]` which is
/// generated by `Segment::from_frame_timestamp_only` for frames
/// captured without subtitles.
...
/// Generate a standalone `slides.html` file at `output_dir/slides.html`.
///
/// Expects `segments` to be in order and `output_dir/images/` to contain
...
/// Load segments from `output_dir/segments.json` and generate slides.html.
...
/// Render the full HTML string for the slides viewer.
pub fn render_slides_html(
    ...
    for seg in segments {
        ...
        writeln!(
            toc_items,
            "<li><a href=\"#slide-{idx}\" data-slide=\"{idx}\" class=\"toc-link\"><span class=\"toc-ts\">{ts}</span><span class=\"toc-preview\">{preview}</span></a></li>",
            idx = seg.index,
            ts = ts,
            preview = preview,
        )?;
...
/// Generate slides.md (Markdown version) alongside slides.html.
...
mod tests {
    ...
    fn toc_anchor_links_match_slide_ids() {
        ...
        // Every TOC link href=#slide-N must have a corresponding id=slide-N
        for seg in &segments {
            let href = format!("href=\"#slide-{}\"", seg.index);
            let id = format!("id=\"slide-{}\"", seg.index);
            assert!(html.contains(&href), "Missing TOC anchor for slide {}", seg.index);
